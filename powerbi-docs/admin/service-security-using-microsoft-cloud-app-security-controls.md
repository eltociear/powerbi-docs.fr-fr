...
Cloud App Security fonctionne avec une architecture de proxy inverse et est intégré à l’accès conditionnel Azure AD pour superviser l’activité des utilisateurs Power BI en temps réel. Les étapes suivantes sont fournies ici pour vous aider à comprendre le processus et des instructions pas à pas détaillées sont fournies dans le contenu lié de chacune de ces étapes suivantes. Vous pouvez également lire cet [article Cloud App Security](/cloud-app-security/proxy-deployment-aad) qui décrit le processus dans son ensemble.

1.  [Créer une stratégie de test d’accès conditionnel Azure AD](/cloud-app-security/proxy-deployment-aad#add-azure-ad)
2.  [Se connecter à chaque application à l’aide d’un utilisateur limité à la stratégie](/cloud-app-security/proxy-deployment-aad#sign-in-scoped)
3.  [Vérifier que les applications sont configurées pour utiliser des contrôles d’accès et de session](/cloud-app-security/proxy-deployment-aad#portal)
4.  [Tester le déploiement](/cloud-app-security/proxy-deployment-aad#step-4-test-the-deployment)

Le processus de définition de stratégies de session est décrit en détail dans l’article [Stratégies de session](/cloud-app-security/session-policy-aad). 
...
Vous pouvez définir des stratégies de détection des anomalies Power BI qui peuvent être étendues indépendamment, afin qu’elles s’appliquent uniquement aux utilisateurs et groupes que vous voulez inclure et exclure dans la stratégie. [En savoir plus](/cloud-app-security/anomaly-detection-policy#scope-anomaly-detection-policies).
...
Les paramètres de ces détections sont configurés dans le portail Cloud App Security. [En savoir plus](/cloud-app-security/anomaly-detection-policy#unusual-activities-by-user). 
...
Dans la fenêtre qui s’affiche, créez la stratégie de session. Les étapes numérotées décrivent les paramètres de l’image suivante.
  ...
  3. Pour **Type de contrôle de session**, sélectionnez *Contrôler le téléchargement du fichier (avec DLP)* .
      ...
      Pour la section **Source de l’activité**, choisissez les stratégies de blocage appropriées. Nous vous recommandons de bloquer les appareils non gérés et non conformes. Choisissez de bloquer les téléchargements quand la session se trouve dans Power BI.
        ...
        ![Création de la stratégie de session – Blocage des téléchargements](media/service-security-using-microsoft-cloud-app-security-controls/cloud-app-security-controls-05.png)
  ...
  7. Veillez à créer une alerte pour une telle action.
        ...
        ![Sélection des paramètres de la stratégie de session](media/service-security-using-microsoft-cloud-app-security-controls/cloud-app-security-controls-06.png)

        

  8. Enfin, prenez soin de sélectionner le bouton **Créer** pour créer la stratégie de session.

        ![Création de la stratégie de session](media/service-security-using-microsoft-cloud-app-security-controls/cloud-app-security-controls-07.png)
...
* [Protéger les applications avec le contrôle d’application par accès conditionnel de Microsoft Cloud App Security](/cloud-app-security/proxy-intro-aad)
* [Déployer le contrôle d’application par accès conditionnel pour les applications proposées](/cloud-app-security/proxy-deployment-aad)
* [Stratégies de session](/cloud-app-security/session-policy-aad)
* [Vue d’ensemble des étiquettes de sensibilité](/microsoft-365/compliance/sensitivity-labels)

...
Vous pouvez créer un calcul DAX qui génère des valeurs qui varient en fonction des conditions de la logique métier que vous sélectionnez. Il est généralement plus rapide de créer une formule DAX que de créer plusieurs règles dans la boîte de dialogue Mise en forme conditionnelle. 

Par exemple, la formule DAX suivante applique des valeurs de couleur hexadécimales à une nouvelle colonne **Affordability rank**, en fonction des valeurs de la colonne existante **Affordability** :

![Calcul DAX](media/desktop-conditional-table-formatting/conditional-table-formatting_05.png)
...
Vous pouvez créer de nombreuses autres variantes en faisant simplement appel à votre imagination et à quelques formules de DAX.

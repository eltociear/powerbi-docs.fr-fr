...
- Votre administrateur Power BI doit avoir autorisé les métriques d’utilisation pour les créateurs de contenu. Il peut aussi avoir activé la collecte des données par utilisateur dans les métriques d’utilisation. Pour plus d’informations, consultez [Activer ces options dans le portail d’administration](service-admin-portal.md#control-usage-metrics).

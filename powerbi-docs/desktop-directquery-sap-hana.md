...
Les deux approches représentent des comportements différents, et il n’est pas possible de passer de l’une à l’autre pour un rapport existant. 
...
Dans le cas d’une connexion à SAP HANA en tant que source multidimensionnelle, les considérations suivantes s’appliquent :
...
* **Connexion à l’aide de SSL** : vous pouvez vous connecter en utilisant l’importation de source multidimensionnelle avec SSL, mais vous ne pouvez pas connecter à SAP Hana des instances configurées pour utiliser SSL pour le connecteur relationnel.

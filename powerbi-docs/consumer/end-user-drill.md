...
![descendre d’un niveau](./media/end-user-drill/power-bi-drill-icon4.png)  ![activer et désactiver l’exploration](./media/end-user-drill/power-bi-drill-icon2.png)  ![icône descendre dans la hiérarchie de tous les champs](./media/end-user-drill/power-bi-drill-icon3.png)
![icône monter dans la hiérarchie](./media/end-user-drill/power-bi-drill-icon5.png) ![icône développer vers le bas](./media/end-user-drill/power-bi-drill-icon6.png)  
...
2. Pour suivre la procédure, [ouvrez l’exemple Retail Analysis](../sample-datasets.md) et créez une tree map qui examine **Total Units This Year** (Values) par **Territory**, **City**, **Postal Code** et **Name** (Group).  La tree map a une hiérarchie constituée de secteurs, de villes, de codes postaux et de noms de ville. Chaque secteur a une ou plusieurs villes, chaque ville a un ou plusieurs codes postaux, etc. Par défaut, la visualisation affiche uniquement les données relatives au secteur, car le *secteur* (Territory) apparaît en premier dans la liste.
   
   ![Sélectionner « Territory »](media/end-user-drill/power-bi-hierarcy-list.png)

2. Comprendre comment les différentes icônes d’exploration fonctionnent ensemble peut prêter à confusion : nous allons donc filtrer la tree map pour afficher seulement 2 des plus petits secteurs : **KY** et **TN**. Sélectionnez la tree map et, sous **Filtres au niveau de l’élément visuel**, développez **Territory** et sélectionnez **KY** et **TN**.

    ![Filtre pour KY et TN](./media/end-user-drill/power-bi-filter.png)    

   Seuls deux secteurs apparaissent maintenant dans la tree map.
...
1. Démarrez au plus haut niveau de la tree map montrant les données pour KY et TN. Élargissez votre tree map en sélectionnant une des poignées et en la faisant glisser vers la droite. 

    ![tree map affichant deux états](./media/end-user-drill/power-bi-drill-down.png) .

2. Pour descendre dans la hiérarchie pour ***tous les champs en même temps***, sélectionnez la flèche double en haut à gauche de la visualisation ![Icône de descente dans la hiérarchie avec deux flèches](./media/end-user-drill/power-bi-drill-icon3.png). Votre tree map montre maintenant les données des villes pour le Kentucky et le Tennessee. 

    ![Icône d’exploration avec deux flèches](./media/end-user-drill/power-bi-drill-down1.png)
   
5. Descendez dans la hiérarchie une nouvelle fois au niveau Postal Code.

    ![Icône d’exploration avec deux flèches](./media/end-user-drill/power-bi-drill-down2.png)

3. Pour remonter d’un niveau, sélectionnez la flèche vers le haut en haut à gauche de la visualisation ![icône Monter d’un niveau](./media/end-user-drill/power-bi-drill-icon5.png).
...
1. Sélectionnez l’icône Descendre dans la hiérarchie pour l’activer ![Fonctionnalité Descendre dans la hiérarchie activée](./media/end-user-drill/power-bi-drill-icon2.png). Vous avez maintenant la possibilité de descendre dans la hiérarchie ***un champ à la fois***. 
   ...
   ![flèche pointant vers l’icône activer/désactiver Descendre dans la hiérarchie](media/end-user-drill/power-bi-drill-icon-new.png)
...
2. À ce stade, vous pouvez continuer à descendre dans la hiérarchie pour Tennessee ou pour une ville particulière du Tennessee, ou bien vous pouvez développer (consultez **Développer tous les champs à la fois** ci-dessous). Continuons à descendre dans la hiérarchie pour un champ à la fois.  Sélectionnez **Knoxville, TN**. Votre tree map montre maintenant le code postal pour votre magasin de Knoxville. 
   ...
   ![la tree map indique 37919](media/end-user-drill/power-bi-drill-down-one2.png)
...
Une tree map qui nous montre uniquement un code postal n’apporte pas beaucoup d’informations.  Développons donc un niveau de la hiérarchie.  

1. Avec la tree map active, sélectionnez l’icône *Développer vers le bas*![Icône Développer vers le bas](./media/end-user-drill/power-bi-drill-icon6.png). Votre tree map montre maintenant les deux niveaux de notre hiérarchie : le code postal et le nom du magasin. 
...
![Barre unique affichant les données regroupées par année](media/end-user-drill/power-bi-hierarchical-axis-datetime-1.png)
...
![graphique à barres pour afficher la barre par mois](media/end-user-drill/power-bi-hierarchical-axis-datetime-2.png)
...
![graphique à barres affichant les unités totales pour cette année par secteur de vente](media/end-user-drill/power-bi-hierarchical-axis-category-1.png)
...
![graphique à barres affichant le niveau le plus bas de la hiérarchie, avec la plupart des détails](media/end-user-drill/power-bi-hierarchical-axis-category-2.png)
...
* Si l’ajout d’un champ de date à une visualisation ne crée pas de hiérarchie, c’est peut-être que le champ « date » n’est pas bien enregistré en tant que date. Si vous êtes le propriétaire du jeu de données, ouvrez-le dans la vue *Données* dans Power BI Desktop, sélectionnez la colonne qui contient la date, puis sous l’onglet Modélisation, définissez **Type de données** sur **Date** ou **Date/Heure**. Si le rapport a été partagé avec vous, contactez le propriétaire pour lui demander d’effectuer la modification.  
  ...
  ![sélectionner la vue des données et, dans l’angle supérieur droit, consulter Type de Date](media/end-user-drill/power-bi-change-data-type2.png)

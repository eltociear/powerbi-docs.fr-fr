---
title: Accéder au journal d’activité de Power BI
description: Conseils et exemple de code de script PowerShell pour travailler avec le journal d’activité de Power BI.
author: peter-myers
ms.reviewer: asaxton
ms.service: powerbi
...
ms.locfileid: "89512545"
---
# <a name="access-the-power-bi-activity-log"></a>Accéder au journal d’activité de Power BI

Cet article s’adresse aux administrateurs Power BI qui doivent accéder au journal d’activité de Power BI. Étant donné qu’il n’existe pas encore d’interface utilisateur pour rechercher le journal d’activité, vous devez utiliser l’API REST et les applets de commande de gestion Power BI.

> [!NOTE]
> Cet article n’introduit pas ou ne décrit pas le journal d’activité de Power BI. Pour plus d’informations, consultez [Suivre les activités utilisateur dans Power BI](../admin/service-admin-auditing.md#use-the-activity-log).

## <a name="powershell-sample"></a>Exemple de code PowerShell

Un exemple PowerShell est disponible pour vous permettre d’apprendre à filtrer et à récupérer des événements du journal d’activité Power BI. Tous les fragments de code et les scénarios sont annotés avec des instructions détaillées et des lacunes ou problèmes courants dont il faut se méfier. Deux scénarios sont abordés :

- Récupérer une liste d’utilisateurs pour une application spécifique.
- Récupérer une liste d’utilisateurs pour le partage de rapport direct.

> [!NOTE]
> Vous devrez vous familiariser avec l’[API d’administration Power BI](/rest/api/power-bi/admin) et les [modules PowerShell Power BI](/powershell/power-bi/overview?view=powerbi-ps). Les modules PowerShell doivent être installés avant l’exécution de ces blocs de script. Pour plus d’informations, consultez [Suivre les activités utilisateur dans Power BI](../admin/service-admin-auditing.md#use-the-activity-log).
>
> Il peut y avoir un décalage de 30 minutes maximum pour récupérer des événements Power BI.

Pour utiliser l’exemple, vous devez répondre aux exigences suivantes :

- Installez les [modules PowerShell Power BI](/powershell/power-bi/overview).
- L’utilisateur du script PowerShell doit se connecter à l’aide de l’[applet de commande Connect-PowerBIServiceAccount](/powershell/module/microsoftpowerbimgmt.profile/connect-powerbiserviceaccount), puis entrer les informations d’identification de l’administrateur du locataire Power BI quand vous y êtes invité. Des privilèges Administrateur de locataire sont nécessaires pour utiliser l’API des journaux d’activité.

> [!IMPORTANT]
> Les utilisateurs ne disposant pas de privilèges Administrateur de locataire ne pourront pas exécuter des fragments de code de l’exemple de script.
...
- [Suivre les activités utilisateur dans Power BI](../admin/service-admin-auditing.md#use-the-activity-log)

...
- L’utilisateur du script PowerShell doit se connecter à l’aide de l’applet de commande [Connect-PowerBIServiceAccount](/powershell/module/microsoftpowerbimgmt.profile/connect-powerbiserviceaccount) et entrer des informations d’identification d’administrateur Power BI quand il y êtes invité. Des privilèges d’administrateur sont nécessaires pour utiliser l’API Activity Log (Journal d’activité).

> [!IMPORTANT]
> Les utilisateurs sans privilèges d’administrateur ne peuvent pas exécuter les fragments de code de l’exemple de script.

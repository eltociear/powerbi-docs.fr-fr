...
### <a name="install-sap-bw-components"></a>Installer les composants SAP BW

Si vous n’avez pas configuré SAP gsskrb5 et gx64krb5 sur votre ou vos machines clientes et sur votre serveur d’applications SAP BW, effectuez cette section. Si vous avez déjà effectué cette configuration (vous avez créé un utilisateur de service pour votre serveur BW et y avez associé un SPN), vous pouvez ignorer certaines parties de cette section.
...
### <a name="complete-the-gateway-configuration-for-sap-bw"></a>Effectuer la configuration de la passerelle pour SAP BW
...
1. Sur un contrôleur de domaine Active Directory, créez un utilisateur de service (initialement un utilisateur Active Directory normal) pour votre serveur d’applications BW dans votre environnement Active Directory. Ensuite, affectez-lui un SPN.

    Le SPN attribué **doit** commencer par SAP/. Ce qui vient après SAP/ ne dépend que de vous ; vous pouvez, par exemple, utiliser le nom de l’utilisateur de service du serveur BW. Ainsi, si vous créez BWServiceUser@\<DOMAIN\> comme utilisateur de service, vous pouvez utiliser le SPN SAP/BWServiceUser. Pour définir le mappage du SPN, vous pouvez recourir à la commande setspn. Par exemple, pour définir le SPN sur l’utilisateur de service que nous venons de créer, vous devez exécuter la commande suivante à partir d’une fenêtre cmd sur une machine de contrôleur de domaine : `setspn -s SAP/ BWServiceUser DOMAIN\ BWServiceUser`.

1. Accordez à l’utilisateur de service l’accès à votre instance de serveur d’applications BW :
...
1. Connectez-vous à votre serveur dans SAP GUI/Logon et définissez les paramètres de profil suivants à l’aide de la transaction RZ10 :
    ...
    1. Définissez le paramètre de profil snc/identity/as sur p:\<l’utilisateur de service BW que vous avez créé\>, par exemple p:BWServiceUser@MYDOMAIN.COM. Notez la présence de « p: » avant l’UPN de l’utilisateur de service.
...
### <a name="map-azure-ad-users-to-sap-bw-users"></a>Mapper les utilisateurs Azure AD aux utilisateurs SAP BW
...
### <a name="verify-sign-in-using-sso"></a>Vérifier la connexion à l’aide de l’authentification unique
...
1. Cliquez avec le bouton droit sur la nouvelle connexion, puis sélectionnez **Properties** (Propriétés). Sélectionnez l’onglet **Réseau**. Dans la fenêtre **SNC Name** (Nom SNC), entrez p:\<UPN de l’utilisateur de service BW\>, par exemple p:BWServiceUser@MYDOMAIN.COM.

    ![Propriétés de l’entrée système](media/service-gateway-sso-kerberos/system-entry-properties.png)

1. Sélectionnez **OK**. À présent, double-cliquez sur la connexion que vous venez de créer pour tenter une connexion SSO au service. Si cette connexion réussit, passez à l’étape suivante. Sinon, passez en revue les étapes antérieures dans ce document pour vous assurer de les avoir effectuées correctement, ou consultez la section de dépannage ci-après. Notez que si vous ne pouvez pas vous connecter au serveur BW via l’authentification unique dans ce contexte, vous ne pouvez pas non plus le faire dans le contexte de la passerelle.
...
### <a name="add-registry-entries"></a>Ajouter des entrées de registre

Ajoutez les entrées de registre requises au registre de la machine sur laquelle est installée la passerelle. Ensuite, définissez les paramètres de configuration de la passerelle requis.
...
1. Dans le champ **Nom du partenaire SNC**, entrez la valeur stockée dans le paramètre de profil snc/identity/as du serveur *avec SAP/ ajouté entre « p: » et le reste de l’identité.* Par exemple, si l’identité snc du serveur est p:BWServiceUser@MYDOMAIN.COM, vous devez entrer p:SAP/BWServiceUser@MYDOMAIN.COM dans la zone d’entrée Nom du partenaire SNC.
...
Publiez un rapport DirectQuery à partir de Power BI Desktop sur Power BI Desktop pour tester votre installation. Vérifiez que vous êtes connecté au service Power BI en tant qu’utilisateur sur lequel vous avez défini la propriété msDS-cloudExtensionAttribute1. Si l’installation a été effectuée correctement, vous devez pouvoir créer un rapport basé sur le jeu de données publié sur le service Power BI et extraire des données via les visuels du rapport.

...
Pour commencer, vous avez besoin d’un compte **Power BI Pro** (il s’agit de votre **compte principal**) et d’un abonnement **Microsoft Azure**.
...
Vous pouvez accéder à [l’outil d’expérience d’intégration](https://aka.ms/embedsetup/AppOwnsData) pour démarrer rapidement et télécharger un exemple d’application qui vous permet de créer un environnement et d’incorporer un rapport.
...
2. Ouvrez le fichier Web.config dans l’exemple d’application. Pour pouvoir exécuter l’application, vous devez renseigner 5 champs : **clientId**, **groupId**, **reportId**, **pbiUsername** et **pbiPassword**.
    ...
    **Recherche d’inscription d’application** Sélectionnez l’application pour laquelle vous souhaitez récupérer la valeur ![clientId](media/embed-sample-for-customers/embed-sample-for-customers-003.png).
...
En créant une capacité dédiée, vous pouvez profiter du fait que vous disposez d’une ressource dédiée pour votre client. Vous pouvez acheter une capacité dédiée dans le [Portail Microsoft Azure](https://portal.azure.com). Pour en savoir plus sur la création d’une capacité Power BI Embedded, consultez [Créer une capacité Power BI Embedded dans le Portail Microsoft Azure](https://docs.microsoft.com/azure/power-bi-embedded/create-capacity).

...
* **Un compte de stockage Azure avec la fonctionnalité Data Lake Storage Gen2 (préversion) activée** : pour la connexion à Azure Data Lake Storage Gen2, vous devez vous inscrire à sa préversion publique
...
Remarque : Patientez au minimum pendant 30 minutes pour que l’autorisation se propage à Power BI à partir du portail. Chaque fois que vous modifiez une autorisation dans le portail, vous devez attendre 30 minutes pour qu’elle soit répercutée dans Power BI avant de réessayer. 
...
Une fois que vous avez configuré votre compte Azure Data Lake Storage Gen2 dans le portail Azure, vous le connectez à Power BI dans le **Portail d’administration Power BI**. Vous gérez également le stockage de flux de données Power BI dans la section des paramètres **Stockage de flux de données (préversion)** du portail d’administration de Power BI. Pour obtenir des conseils sur le lancement et l’utilisation de base, consultez [Guide pratique pour accéder au portail d’administration](service-admin-portal.md).

Vous connectez votre compte **Azure Data Lake Storage Gen2** en procédant comme suit :

1. Accédez à l’onglet **Paramètres du flux de données (préversion)** du **Portail d’administration de Power BI**

    ![Portail d’administration Power BI](media/service-dataflows-connect-azure-data-lake-storage-gen2/dataflows-connect-adlsg2_08.jpg) 

2. Sélectionnez le bouton **Connecter votre Azure Data Lake Storage Gen2 préversion**. La fenêtre suivante s’affiche.
...
3. Ben reçoit une erreur similaire à l’image suivante, car il n’a pas été autorisé à accéder au dossier CDM du flux de données dans le Data Lake

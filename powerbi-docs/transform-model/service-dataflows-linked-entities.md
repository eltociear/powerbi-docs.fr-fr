...
Les entités liées nécessitent un abonnement [Power BI Premium](../admin/service-premium-what-is.md) à actualiser. Les entités liées sont disponibles dans n’importe quel flux de données sur un espace de travail hébergé sur la capacité Power BI Premium. Il n’existe aucune limitation concernant le flux de données source.

Les entités liées fonctionnent uniquement correctement dans les nouveaux espaces de travail Power BI. Si vous le souhaitez, des [informations supplémentaires sur les nouveaux espaces de travail Power BI](../collaborate-share/service-create-the-new-workspaces.md) sont disponibles. Pour fonctionner correctement, tous les flux de données liés doivent se trouver dans les nouveaux espaces de travail.
...
* Le flux de données doit résider dans un [nouvel espace de travail Power BI](../collaborate-share/service-create-the-new-workspaces.md)
...
* Quand une requête (par exemple, Requête A) est utilisée dans le calcul d’une autre requête (Requête B) dans des dataflows, la requête B devient une entité calculée. Les entités calculées ne peuvent pas faire référence à des sources locales.
